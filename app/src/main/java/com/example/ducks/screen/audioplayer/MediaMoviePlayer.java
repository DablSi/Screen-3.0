package com.example.ducks.screen.audioplayer;
/*
...
public class MediaMoviePlayer {
	...
	public MediaMoviePlayer(@NonNull final IFrameCallback callback, final boolean audio_enable) {
...
//--------------------------------------------------------------------------------
	...
	private final void handlePrepare(final String source_file) throws IOException {
		...
		if (mAudioTrackIndex < 0) {
			throw new RuntimeException("No audio track found in " + source_file);
		}
		synchronized (mSync) {
	...
	protected void updateMovieInfo() {
		String value = mMetadata.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION);
